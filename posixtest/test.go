...
func RenameOpenDir(t *testing.T, mnt string) {
	...
	if err := syscall.Fstat(fd, &st2); err != nil {
		t.Skipf("Fstat failed: %v. Known limitation - see https://github.com/SuzukiHonoka/go-fuse/issues/55", err)
	}
	...
	if st2.Mode&0777 != st1.Mode&0777 {
		t.Skipf("got permissions %#o, want %#o. Known limitation - see https://github.com/SuzukiHonoka/go-fuse/issues/55",
